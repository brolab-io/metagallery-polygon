...
type Result = {
  call: (tokenId: string | BigNumber, poolId: string | BigNumber) => Promise<void>;
  isLoading: boolean;
};

const useStakeNftToPool = () => {
  const [isLoading, setIsLoading] = useState(false);

  const call: Result["call"] = useCallback(async (tokenId, poolId) => {
    if (!window.ethereum) {
    ...
    const abi = ["function stake(uint256,uint256)"];
    ...
    try {
      ...
      const tx = await contract.stake(tokenId, poolId);
...
export default useStakeNftToPool;
